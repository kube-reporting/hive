...
RUN mvn -X -B -e -T 1C -DskipTests=true -DfailIfNoTests=false -Dtest=false clean package -Pdist
...
RUN ln -s /usr/share/java/postgresql-42.2.16.jar "$HIVE_HOME/lib/postgresql-jdbc.jar"
